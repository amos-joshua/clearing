...
from clearing_server.core.model.events import SenderCallInit, ReceiverAck, ReceiverReject, CallError, SenderAuthorize
...
async def test_call_timeout_over_websocket(mock_users: FirebaseEmulatorUserManager, users: UserRepositoryFirebase, config: ServerConfig):
    ...
    try:
        async with ScriptedWebsocketClient().connect(endpoint) as client:
            # WHEN
            # we authorize and then send a caller init and wait 1s
            await client.send([
                SenderAuthorize(
